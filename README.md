...
2.  **Frontend (React):**
    ...
    - Implement UI controls to allow users to:
      - **Filter by:**
        ...
        - `Teams` - Allow selecting one or multiple teams (show users in _any_ selected team).
...
2. Install dependencies (three separate installations are required):
   ...
   **Alternatively**, you can use the provided setup script that handles all three installations:

   ```bash
   npm run setup
   ```

3. Set up the database (if not done already):
   ```bash
   npm run prisma:generate
   npm run prisma:migrate
   ```
